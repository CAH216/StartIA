...
export async function GET() {
  ...
  const mySlotKeys = new Set(mySchedule.map((s: { dayOfWeek: number; hour: number }) => `${s.dayOfWeek}-${s.hour}`));
  ...
  const myPending = pendingBookings.filter((b: { bookedDate: Date | null; bookedHour: number | null }) => {
    if (!b.bookedDate || b.bookedHour == null) return false;
    const dow = (new Date(b.bookedDate).getDay() + 6) % 7;
  ...
  // Dédupliquer
  const assignedIds = new Set(myAssigned.map((b: { id: string }) => b.id));
  const combined = [
    ...myAssigned,
    ...myPending.filter((b: { id: string }) => !assignedIds.has(b.id)),
  ];
